...
def _batch_hard(mat_distance, mat_similarity, indice=False):
	sorted_mat_distance, positive_indices = torch.sort(mat_distance, dim=1, descending=False)
	...
	#hard_p = sorted_mat_distance[:, 0]
	#hard_p_indice = positive_indices[:, 0]
	hard_n = sorted_mat_distance[:, 50]
	hard_n_indice = positive_indices[:, 50]
	'''
...
class TripletLoss(nn.Module):
	...
	def forward(self, emb,emb2, label):
		if self.normalize_feature:
			...
			emb = F.normalize(emb)
		#mat_dist = euclidean_dist(emb.clone().detach(), emb2.clone().detach())
		mat_dist =1-emb.mm(emb2.t())
		#mat_dist =cosine_dist(emb, emb2)
		#print(mat_dist,mat_dist.size())
		# mat_dist = cosine_dist(emb, emb)
		assert mat_dist.size(0) == mat_dist.size(1)
		N = mat_dist.size(0)
		mat_sim = 0
		dist_ap, dist_an = _batch_hard(mat_dist.clone().detach(), mat_sim)
